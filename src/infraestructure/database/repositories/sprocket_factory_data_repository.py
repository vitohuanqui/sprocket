...
async def get_all(
    ...
    if 'group_by' in query_params and query_params['group_by'] in MAPPING:
        ...
        select_list += [
            extract(x, Model.c.time).label(x)
            for x in MAPPING[query_params['group_by']]
        ]
...
async def get_all_by_sprocket_type_and_factory(
    sprocket_type: SprocketType, factory: Factory
) -> Iterable[SprocketFactoryData]:
    query = Model.select().where(
        Model.c.sprocket_type_id == sprocket_type.id,
        Model.c.factory_id == factory.id,
    )
    result = await database.fetch_all(query)
...
async def create(
    dto: CreateSprocketFactoryDataDto, is_goal: bool = False
) -> SprocketFactoryData:
...
async def update(
    dto: UpdateSprocketFactoryDataDto, id_: int
