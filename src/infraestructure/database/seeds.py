...
from src.infraestructure.database.sqlalchemy import (database_context,
                                                     init_database,
                                                     truncate_database)
...
async def _populate_data(db: Database) -> None:
    ...
    f.close()
    values_factory = [
        {'name': 'Factory #1', 'url': 'third-app:8000/data'},
        {'name': 'Factory #2', 'url': 'third-app:8000/data'},
    ]
    ...
    for idx_factory, _ in enumerate(values_factory):
        for idx_spr_type, _ in enumerate(values_sprocket_type):
            await _populate_sprocket_factory_data(
                db, idx_factory, idx_spr_type
            )


async def _populate_sprocket_factory_data(
    db: Database, factory_id: int, sprocket_type_id: int
) -> None:
    ...
    for factory in json_values:
        for idx, val in enumerate(
            factory['factory']['chart_data']['sprocket_production_actual']
        ):
            values.append(
                {
                    'production': val,
