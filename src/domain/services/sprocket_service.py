...
async def create(
    repository: SprocketTypeRepository, dto: CreateSprocketTypeDto,
) -> SprocketType:
...
async def get(repository: SprocketTypeRepository, id_: int) -> Optional[SprocketType]:
    return await repository.get(id_)


async def get_all(repository: SprocketTypeRepository) -> Iterable[SprocketType]:
