...
from src.domain.entities.sprocket_factory_data import CreateSprocketFactoryDataDto
...
sp_factory_data_repository = get_dependencies().sprocket_factory_data_repository
...
def _get_sprocket_factory_data():
    ...
    factories = loop.run_until_complete(factory_service.get_all(factory_repository))
    sp_types = loop.run_until_complete(sprocket_service.get_all(sprocket_type_repository))
    for factory in factories:
        response_data = repository.fetch_data(factory.url)
        for sp_type in sp_types:
            data.append(CreateSprocketFactoryDataDto(**{
